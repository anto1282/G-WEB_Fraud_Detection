...
class GCN(torch.nn.Module):
    def __init__(
        self, in_channels: int, hidden_channels: int, out_channels: int, heads: int, dropout: float
    ) -> None:
        super().__init__()
        
        self.dropout = dropout
        self.conv1 = GATConv(in_channels, hidden_channels, heads, dropout=dropout)
        self.conv2 = GATConv(hidden_channels * heads, int(hidden_channels/4), heads=1, concat=False, dropout=dropout)
...
if __name__ == "__main__":
    ...
    x = torch.randn(5, in_channels)  # Node features (5 nodes, 16 features each)
    edge_index = torch.tensor([[0, 1, 2, 3], [1, 0, 3, 4]], dtype=torch.long)  # Edge list (4 edges)
